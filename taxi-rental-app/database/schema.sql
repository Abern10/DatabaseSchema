...
-- Alex
-- This is all from app Database.sql I just pasted it over to this file and deleted the other so its organized

-- Update: There was an error with the schema, I added UNIQUE (model_id) constraint to the Model table.

-- To create the database go into PGadmin 4
-- Servers (on left column) -> Right click on databases -> Create -> Database
-- -> Name it whatever, leave rest as defualt and save -> Query Tool(top left)
-- -> Past schema into editior and then exectute, to connect app to db update .env file
...
CREATE TABLE Model (
    ...
    -- Add a unique constraint on model_id alone for foreign key reference
    UNIQUE (model_id),
